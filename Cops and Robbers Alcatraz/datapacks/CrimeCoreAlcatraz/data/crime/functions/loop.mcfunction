...
# Escaping players
execute if score crime_gamestate value matches 1.. at @e[limit=1,type=minecraft:armor_stand,tag=crime_marker_escape] as @a[tag=!ignore.global,team=prisoners,distance=..11] run function crime:escapeme
...
execute at @e[tag=crime_marker_returntotop] if block ~ ~ ~ minecraft:stone_pressure_plate[powered=true] as @a[tag=!ignore.global,distance=..1] run teleport @s @e[tag=crime_marker_returntotop_loc,limit=1] 


# Only for COPS 3 <----------
execute if block 64 141 -29 minecraft:stone_button[powered=true] run scoreboard players set crime_secret1 value 1
execute if block 69 148 -75 minecraft:stone_button[powered=true] run scoreboard players set crime_secret2 value 1
execute if score crime_secret1 value matches 1 run title @a[tag=!ignore.global] actionbar {"text":"A noise of a lock turning is heard throughout the prison"}
execute if score crime_secret2 value matches 1 run title @a[tag=!ignore.global] actionbar {"text":"A noise of a hinge turning is heard throughout the prison"}
execute if score crime_secret1 value matches 1 run scoreboard players set crime_secret1 value 2
execute if score crime_secret2 value matches 1 run scoreboard players set crime_secret2 value 2
execute if score crime_secret1 value matches 2 if score crime_secret2 value matches 2 run setblock 94 147 19 air
execute if score crime_secret1 value matches 2 if score crime_secret2 value matches 2 run title @a[tag=!ignore.global] actionbar {"text":"The secret passage has been opened!"}
execute if score crime_secret1 value matches 2 if score crime_secret2 value matches 2 run scoreboard players set crime_secret1 value 3

# Teleportation loop
execute at @e[type=minecraft:armor_stand,tag=crime_marker_warden_teleport_office_trigger] run teleport @a[tag=!ignore.global,team=wardens,distance=..1] @e[limit=1,type=armor_stand,tag=crime_marker_warden_teleport_office]
execute at @e[type=minecraft:armor_stand,tag=crime_marker_warden_teleport_tower_trigger] run teleport @a[tag=!ignore.global,team=wardens,distance=..1] @e[limit=1,type=armor_stand,tag=crime_marker_warden_teleport_tower]

# crime_marker_warden_teleport_(Loc) - location to be teleported to
# crime_marker_warden_teleport_(Loc)_trigger - location to trigger teleportation
# Only for COPS 3 <----------