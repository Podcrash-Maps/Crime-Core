...
# Messages to be sent
execute if score crime_gametype value matches 1 run title @a[tag=!ignore.global,team=escapees] actionbar {"text":"All prisoners escaped!","color":"green"}
execute if score crime_gametype value matches 1 run title @a[tag=!ignore.global,team=wardens] actionbar {"text":"All prisoners escaped!","color":"red"}
